              ...
              {currentPost.map(currentPost => (
                <Col className="gutter-row" span={6}>
                  <Card style={{ width: 240 }} cover={<img key={currentPost.id} />}>
                    ...
                    <br />
                    <Button style={{ backgroundColor: '#52c41a', color: 'white' }} shape="round" size={size} onClick={this.showModification}>
                      Modifier l'article
                    </Button>
                    <Modal visible={visible2} onOk={() => this.handleOk(currentPost.id)} confirmLoading={confirmLoading} cancelText='annuler' onCancel={this.handleCancel}>
                      <ExclamationCircleOutlined size={size} />
                      <p>{ModalText}</p>
                    </Modal>
                  </Card>
