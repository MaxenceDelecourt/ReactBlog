            ...
            render: (text, record, index) =>
              <div>
                <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }}>
                  <Col span={12}>
                    <Button type="primary" shape="round" size={size} danger onClick={() => this.showModal(index, true)}>
                      <DeleteOutlined />supprimer
                    </Button>
                  </Col>
                  <Col span={12}>
                    <Button style={{ backgroundColor: '#52c41a', color: 'white' }} shape="round" size={size} onClick={() => this.showModal(index, false)}>
          ...
          <div className="site-layout-content">
            ...
            <Modal visible={visible} onOk={() => this.handleOk(dataSource[index].id)} confirmLoading={confirmLoading} cancelText='annuler' onCancel={() => this.handleCancel()}>
              {typeModal
                ...
                V<ModifyPost title={dataSource[index].title} description={dataSource[index].description} slug={dataSource[index].slug} content={dataSource[index].content} date={dataSource[index].date} />
                :
