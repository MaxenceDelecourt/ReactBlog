...
class Post extends Component {
  ...
  render() {
    const { error, isLoaded, post, size, currentPage, postPerPage, visible, confirmLoading, ModalText, options } = this.state
            ...
            render: (index) =>
              <div>
                <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }}>
                  <Col span={12}>
                    <Button type="primary" shape="round" size={size} danger onClick={() => this.showModal(index, '1')}>
                      <DeleteOutlined />supprimer
                    </Button>
                  </Col>
                  <Col span={12}>
                    <Button style={{ backgroundColor: '#52c41a', color: 'white' }} shape="round" size={size} onClick={() => this.showModal(index, '0')}>
        ...
        return (
          <div className="site-layout-content">
            ...
            <Modal visible={visible} onOk={() => this.handleOk(record.id)} confirmLoading={confirmLoading} cancelText='annuler' onCancel={() => this.handleCancel('1')}>
              ...
              <ModifyPost title={datasource[index].title} description={record.description} slug={record.slug} content={record.content} date={record.date} />
            </Modal>
            <br />
            <br />
