...
async componentDidMount() {
  ...
  render(){
    const { error, isLoaded, post, size, currentPage } = this.state
    const indexOfLastPage = currentPage * post.length
    const indexOfFirstPage = currentPage - post.length
    const currentPost = post.slice(indexOfFirstPage)
    ...
    if (error !== null) {
      ...
      return (
        <div className="site-layout-content">
            ...
            <Pagination defaultCurrent={6} total={post.length} />
          </div>
      );
    }
  }
