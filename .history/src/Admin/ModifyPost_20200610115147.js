...
import { Form, Input, Button, Col, Upload, Select, DatePicker } from 'antd';
...
class ModifyPost extends Component {
    ...
    async componentDidMount() {
        ...
        this.setState({idAuthor: result[0].id, author: result[0].name})
    }
    ...
    render() {
        const {title, description, slug, content, author, idAuthor, date} = this.state
                ...
                <Form name="basic" onFinish={this.onFinish} onFinishFailed={this.onFinishFailed}>
                    <Col className="gutter-row" span={12} offset={6}>
                        ...
                        <Form.Item name="author" label="Auteur" rules={[{ required: true, },]}>
                            <Select name="author" value={idAuthor} placeholder="Selectioner un auteur d'article" onChange={(value) => this.setState({ author: value })} allowClear>
                                <Option value={idAuthor}>{author}</Option>
                            </Select>
                        </Form.Item>
                        <br />
