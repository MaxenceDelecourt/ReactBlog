...
const cli = await.getSpace('8oamdugxcf33')
...
export async function getPostDescription(slug) {
    try {
        const cli = await cli.getSpace('8oamdugxcf33')
        const response = await client.getEntries({ content_type: 'post', 'fields.slug': slug })
        return response
    } catch (err) {
        return 'error'
    }
